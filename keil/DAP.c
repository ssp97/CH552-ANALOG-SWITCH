...
static UINT8 DAP_Info(UINT8 id, UINT8 *info)
{
    UINT8 length = 0U;
...
static UINT8 DAP_Delay(const UINT8 *req, UINT8 *res)
{
    UINT16 delay;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static UINT8 DAP_HostStatus(const UINT8 *req, UINT8 *res)
{
...
//             number of bytes in request (upper 16 bits)
UINT8I debug_port;
static UINT8 DAP_Connect(const UINT8 *req, UINT8 *res)
{
    UINT8 port;
...
static UINT8 DAP_Disconnect(UINT8 *res)
{
...
//   response: pointer to response datas
//   return:   number of bytes in response
static UINT8 DAP_ResetTarget(UINT8 *res)
{
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static UINT8 DAP_SWJ_Pins(const UINT8 *req, UINT8 *res)
{
    UINT8 value;
    UINT8 select;
    UINT16 wait;
    ...
    value = (UINT8) * (req + 0);
    select = (UINT8) * (req + 1);
    wait = (UINT16)(*(req + 2) << 0) | (UINT16)(*(req + 3) << 8);
    if ((UINT8)(*(req + 4)) || (UINT8)(*(req + 5)))
        wait |= 0x8000;
    ...
    value = ((UINT8)SWK << DAP_SWJ_SWCLK_TCK) |
            ((UINT8)SWD << DAP_SWJ_SWDIO_TMS) |
            (0 << DAP_SWJ_TDI) |
            (0 << DAP_SWJ_TDO) |
            (0 << DAP_SWJ_nTRST) |
            ((UINT8)RST << DAP_SWJ_nRESET);
...
UINT8I fast_clock;
UINT8I clock_delay;
static UINT8 DAP_SWJ_Clock(const UINT8 *req, UINT8 *res)
{
    /**/
    fast_clock = *req;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static UINT8 DAP_SWJ_Sequence(const UINT8 *req, UINT8 *res)
{
    UINT8 count;
...
UINT8I turnaround;
UINT8I data_phase;
static UINT8 DAP_SWD_Configure(const UINT8 *req, UINT8 *res)
{
    UINT8 value;
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static UINT8 DAP_SWD_Sequence(const UINT8 *req, UINT8 *res)
{
    UINT8 sequence_info;
    UINT8 sequence_count;
    UINT8 request_count;
...
UINT16I retry_count;
UINT16I match_retry;
static UINT8 DAP_TransferConfigure(const UINT8 *req, UINT8 *res)
{

    idle_cycles = *(req + 0);
    retry_count = (UINT16) * (req + 1) |
                  (UINT16)(*(req + 2) << 8);
    match_retry = (UINT16) * (req + 3) |
                  (UINT16)(*(req + 4) << 8);
...
static UINT8 DAP_SWD_Transfer(const UINT8 *req, UINT8 *res)
{
    ...
    *(response_head + 1) = (UINT8)response_value;
    ...
    return ((UINT8)(res - response_head));
}
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static UINT8 DAP_Transfer(const UINT8 *req, UINT8 *res)
{
    UINT8 num = 0;
...
//   response: pointer to response datas
//   return:   number of bytes in response
static UINT8 DAP_SWD_TransferBlock(const UINT8 *req, UINT8 *res)
{
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
static UINT8 DAP_TransferBlock(const UINT8 *req, UINT8 *res)
{
    UINT8 num;
    ...
    else
    {
        ...
        num |= (4U + (((UINT8)(*(req + 1)) | (UINT8)(*(req + 2) << 8)) * 4)) << 16;
    }
...
//   response: pointer to response datas
//   return:   number of bytes in response
static UINT8 DAP_SWD_WriteAbort(const UINT8 *req, UINT8 *res)
{
    UINT8 datas[4];
...
//   return:   number of bytes in response (lower 16 bits) 
//             number of bytes in request (upper 16 bits) 
static UINT8 DAP_WriteAbort(const UINT8 *req, UINT8 *res)
{
    UINT8 num;
...
void DAP_Thread(void)
{
    ...
    if (Ep2Oi != Ep2Oo)
    {
        ...
        if (Ep2Oo >= DAP_PACKET_COUNT)
            Ep2Oo = 0;
        if (*req >= ID_DAP_Vendor0 && *req <= ID_DAP_Vendor31)
        {
            *res = ID_DAP_Invalid;
            num = 1;
        }
